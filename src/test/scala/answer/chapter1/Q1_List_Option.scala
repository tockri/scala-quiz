...
/**
  *
  */
class Q1_List_Option extends FlatSpec {
  /**
    * 要素のOptionのうち、Someのものだけを返すメソッドを作りましょう
    */
  def removeOption[A](optList:List[Option[A]]): List[A] = optList.flatMap(oa => oa)
  ...
  /**
    ...
    */
  def addAll(optList:List[Option[Int]]):Int = removeOption(optList).sum

  /**
    * addAllのテスト
    */
  it should "add all List[Option[A]]" in {
    assert(addAll(List(Some(1), Some(2), Some(3), Some(4))) == 10)
